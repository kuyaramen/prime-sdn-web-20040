              ...
              <StaggerItem key={mentor.slug}>
                <Link href={`/education-talent/mentors/${mentor.slug}`}>
                  <motion.div whileHover={{ y: -8, scale: 1.02 }} className="rounded-2xl p-6 border cursor-pointer h-full" style={{ background: "linear-gradient(135deg, #FFFFFF 0%, #F8FAFF 100%)", borderColor: "#E5E7EB", boxShadow: "0 20px 50px rgba(17,24,39,0.08)" }}>
                    ...
                    <h3 className="font-bold text-lg mb-1 text-center" style={{ fontFamily: "Montserrat, Arial, sans-serif", color: "#1A2F6B" }}>{mentor.name}</h3>
                    <p className="text-sm text-center mb-2" style={{ color: "#5E7CE2" }}>{mentor.role}</p>
                    <p className="text-sm text-center mb-4" style={{ color: "#6B7280" }}>{mentor.organization}</p>
                    <div className="flex flex-wrap gap-2 mb-4 justify-center">
                      {mentor.expertise.slice(0, 2).map((exp, i) => (
                        <span key={i} className="px-2 py-1 rounded-full text-xs" style={{ backgroundColor: "#EAF2FF", color: "#3565D9" }}>{exp}</span>
                      ))}
                    </div>
                    <Button variant="outline" className="w-full py-2 rounded-full border-2 border-[#3565D9] text-[#3565D9] hover:bg-[#3565D9] hover:text-white">View Profile <ChevronRight className="w-4 h-4 ml-2" /></Button>
                  </motion.div>
                </Link>
              </StaggerItem>
            ))}
